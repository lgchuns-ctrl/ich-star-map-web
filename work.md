...
## 2026-08-07 星系坐标偏移 Bug 修复

- 问题：用户反馈类别星系“显示有问题”——星点全部挤在画布左上角，仅 2 颗可见；
...
## 2026-08-07 方向修正：彻底改为 2D 星云（第一阶段静态 Overview）

- 用户否决 3D 星球方向，要求**彻底 2D 星云数据可视化**；删除 `CategoryGalaxy3D.vue`、`CategoryGalaxy.vue`，新建 `CategoryNebula.vue`（Canvas 2D，无任何 3D/手势/拖拽/滚轮）；
- 静态 Overview（第一阶段，交互留待验收后）：
  - 背景：深蓝黑径向渐变 + 2000 远景恒星（轻微闪烁）+ 5 片离屏生成的蓝紫星云（screen 混合、缓慢漂移）+ 1500 全画布微尘 + 3200 银河星尘；
  - 数据：3610 个真实地区子项 = 3610 个数据星点（dataParticles），按十大类别布局为 10 个不规则星云区域（椭圆/旋涡/带状/弧形 + Gaussian + Noise + 旋涡扰动 + 全局银河旋转/压扁），类别区域大小按子项数 sqrt 压缩；
  - 装饰粒子（decorativeParticles：背景恒星/星尘/微尘）与数据粒子严格分离；固定随机种子（subitem_id 哈希），刷新后位置稳定；
  - 颜色：深蓝/青蓝/靛青/紫蓝环境 + 十大类别低饱和 Accent，星点 0.5–1.4px、透明度 0.15–0.8，无大面积白色 Glow；
  - 标签：10 个 DOM 标签（英文小标 + 中文名 + 子项数）悬浮于星云区域，移动端仅显示名称；
- 验证：CDP 像素统计——97.7% 画布有内容、白色过热 0%、标签 10 个、控制台 0 异常；type-check / 95 测试 / build 通过；无 three 依赖残留。

## 2026-08-07 方向再修正：2.5D 固定视角宇宙图（静态 Overview）

- 用户明确不要“纯 2D 散点”也不要“自由 3D”，要求**2.5D 固定视角**：十个类别星团在同一银河主平面内围绕分布，带轻微前后层次与弱透视，但无旋转/手势/拖拽/滚轮；
- `CategoryNebula.vue` 重写：加入盘面倾斜（TILT=0.3）+ 弱透视投影（F=1500）+ 每类 z 深度（±0–38）；类别布局为中心（传统技艺/民俗）+ 周围八类；鼠标仅提供极轻微视差；
...
## 2026-08-07 总览视觉调整：银河背景 + 环形排列

- `CategoryGalaxy3D.vue`：新增离屏生成的旋臂银河纹理（亮核+3 旋臂+椭圆星尘+模糊），作为倾斜大平面置于场景最底层并缓慢自转；
- 十个天体改为 XZ 平面半径 250 的环形均匀排列（±6 错位）；
- 验证：CDP 实测半径 245–257、galaxyPlane 纹理与尺寸正确、无手势按钮、0 异常；type-check / build 通过。

## 2026-08-07 详情模式：粒子球体 + 隐藏银河背景

- 进入类别后银河盘面透明度平滑降至 0（保留深空星点）；
- 详情粒子改为 3D 高斯球体（半径 125–150，边缘清晰，缓慢自转）；相机绕球心旋转，拖拽即转球；
- 粒子 Hover/Click 交互、按批次/省份着色、返回动画保持不变；镜头目标进出时平滑插值；
- 验证：CDP 实测 629 粒子、球体最大半径 150、详情银河 opacity≈0、拖拽 theta 变化、返回恢复；type-check / build 通过。

## 2026-08-07 新版 3D 星系上线

- 构建 → 更新 gh-pages 分支（3598331）→ 推送上线；
- 线上验证：HTTP 200，bundle index-By4xzscp.js，界面关键文字校验通过；
- 线上包含：银河系背景 + 十天体环形排列总览；点击进入后银河隐藏、粒子球体可拖拽旋转、粒子 Hover/Click 接详情；返回恢复。
